...
describe("Тестирование авторизации и профиля", () => {
  ...
  it("Верный логин, неверный пароль", () => {
    ...
    cy.contains("button", "Войти").click();
    cy.contains("Введенный пароль недействителен.", { timeout: 10000 }).should(
      "be.visible"
    );
  });
  ...
  it('Переход в раздел "Учебный центр" и далее в подраздел "Компания"', () => {
    cy.login(data.validLogin, data.validPassword);
    cy.get('[data-cy="submenu-title-ms-education-center"]')
      .should("be.visible")
      .click();
  ...
  it("Сравнение имени руководителя и имени авторизованного пользователя", () => {
    ...
    cy.get('div.description-value a[target="_blank"]')
      .invoke("text")
      .then((leaderName) => {
        cy.get(
          "span.el-avatar.el-avatar--circle.cursor-pointer.avatar-icon-bg.el-tooltip__trigger"
        )
          .should("be.visible")
          .click();
        cy.get(".font-bold.text-center")
          .invoke("text")
          .then((profileName) => {
